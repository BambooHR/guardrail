		...
		if ($catch->var) {
			...
			if ($scope->getVarExists($name)) {
				...
				if (!$allPreviousTypesAreExceptions) {
					$scopeStack->getOutput()->emitError(__CLASS__, $scopeStack->getCurrentFile(), $node->getLine(), ErrorConstants::TYPE_EXCEPTION_DUPLICATE_VARIABLE, "Catch overwrites existing variable (\$$name " . TypeComparer::typeToString($oldType) . ") as $newTypes");
				}
			}
