...
has grown up, you will start to run into difficulties dealing with a large code base in PHP. Many of these 
complications are due to the fact that PHP is a weakly typed language. The lack of enforcement of 
...
Also, as you start to use more strongly typed improvements to PHP, you discover that those errors are
...
applied to any PHP 5 - PHP 8 code base. 
...
| Standard.VariableVariable           | Referencing a variable with $$var                                                                               |
 ...
 the abstract syntax tree to enable additional checks. At BambooHR, we use this plugin mechanism
 to run additional checks that are only relevant to our stack.
 ...
 - PHP allows you to declare a function inside another function. This nested function
 ...
 - Guardrail does not conditionally process functions. If a function is defined either at 
 the top level or nested in a function, it will be indexed and considered globally available.
 - Guardrail relies upon reflection to determine the availability of internal PHP methods and functions.
 ...
 Guardrail is available as a Composer package: BambooHR/Guardrail.
...
### Analysis
Once the index is produced, the analysis can be run. Analysis is heavily CPU bound.
...
Guardrail configuration consists of seven sections:
...
many DocBlocks that actually reference types that don't exist 
or aren't namespaced correctly. By default, DocBlocks will not
be used in type inference. If you enable DocBlocks, Guardrail can be
...
The *index* section is a list of subdirectories to index.
 ...
 ignore section can use globbing patterns, including double asterisks,
 ...
 The *test-ignore* section is a list of file paths to ignore from analysis. This section
 ...
 The *plugins* section is a list of plugins to use in the analysis.
...
in an sqlite database, use the following command line.
...
is a helpful feature for incrementally improving your codebase. You can, for example, set:
<pre> 
{
...
This will emit a "Standard.VariableVariable" error only when the error occurs in the
patchset that you are testing. At BambooHR, we have wired this into our RapidCI
...
Languages like Java or C# support casting an object reference from one type 
 ...
 interfaces from one interface to another. The nature of the object hasn't changed, 
